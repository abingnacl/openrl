...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(
        ...
        # configure spaces
        self.all_action_spaces = []
        self.all_observation_spaces = []
        self.all_critic_observation_spaces = []
        critic_obs_dim = 0
        for agent in self.agents:
            ...
            if len(total_action_space) > 1:
                ...
                self.all_action_spaces.append(act_space)
            else:
                self.all_action_spaces.append(total_action_space[0])

            # observation space
            obs_dim = len(observation_callback(agent, self.world))
            critic_obs_dim += obs_dim
            self.all_observation_spaces.append(
                spaces.Box(
        ...
        # TODO
        critic_observation_space = self.all_critic_observation_spaces[0]
        policy_observation_space = self.all_observation_spaces[0]
        self.observation_space = gym.spaces.Dict(
            {
                "policy": policy_observation_space,
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.all_action_spaces[i])
        # advance world state
    ...
    def reset(
        ...
        *,
        ...
        self.current_step = 0
        # reset world
        self.reset_callback(self.world, self._np_random)
        # reset renderer
        self._reset_render()
    ...
    def _set_action(self, scalar_action, agent, action_space, time=None):
        onehot_action = np.zeros(self.action_space.n)
        onehot_action[scalar_action] = 1
        ...
        if isinstance(action_space, MultiDiscrete):
            ...
            index = 0
            for s in size:
                act.append(onehot_action[index : (index + s)])
                index += s
            action = act
        else:
